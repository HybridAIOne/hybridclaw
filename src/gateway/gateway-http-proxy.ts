...
async function resolveHubSpotOAuthTokenOrThrow(
  ...
  return await resolveOAuthTokenOrThrow({
    ...
    loginHint: `${secretName} is not available. Store a HubSpot Service Key with \`/secret set HUBSPOT_ACCESS_TOKEN <token>\`, or run \`hybridclaw auth login hubspot --access-token <token>\`.`,
  });
}
