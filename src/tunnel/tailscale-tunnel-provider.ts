    ...
    try {
      ...
      if (!authKey) {
        if (isTailscaleLoggedOutError(message)) {
          throw new Error(
            `tailscale is not logged in and ${this.tokenSecretName} is not configured in encrypted runtime secrets. Store it with \`hybridclaw secret set ${this.tokenSecretName} <authkey>\` or in TUI with \`/secret set ${this.tokenSecretName} <authkey>\`, or run \`tailscale login\` on the host.`,
          );
        }
        throw error;
      }
