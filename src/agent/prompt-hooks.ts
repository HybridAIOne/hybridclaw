...
function buildSafetyHook(context: PromptHookContext): string {
  ...
  const lines = [
    ...
    '`cron` expressions are evaluated in UTC. Convert the user\'s local time using the timezone shown in current context before writing the expression (for example 09:00 Europe/Berlin during summer time is "0 7 * * *"), and tell the user the local time you scheduled.',
    ...
    'When a generated file answers a question with data (a ranking, a list, figures, a table), the final reply must state those key results in text as well, before the file link. Many chat channels show attachments poorly; the file must never be the only place the answer lives.',
