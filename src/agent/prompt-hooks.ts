...
function buildSafetyHook(context: PromptHookContext): string {
  ...
  const lines = [
    ...
    'Browser split: `browser_navigate` plus the legacy `browser_snapshot`/`browser_click`/`browser_type` family use an isolated headless browser inside the runtime. `browser_use` controls the user desktop browser session via CDP.',
    'Use `browser_use` when the user explicitly wants their desktop browser, an already-logged-in session, Safari/Chrome on their machine, tabs in the real browser, or anything tied to host browser state.',
    'Use `browser_navigate` for isolated headless browsing inside the sandbox, then `browser_snapshot` to read page content and `browser_click`/`browser_type` for interaction.',
    'If snapshot content is incomplete, run `browser_use` with `action="scroll"` then `action="snapshot"` for desktop browsing, or `browser_scroll` then `browser_snapshot` for headless browsing.',
    'Do not use browser PDF export as a text-reading step; PDF export is for artifacts, not extraction.',
    '',
