...
function buildSafetyHook(context: PromptHookContext): string {
  ...
  const lines = [
    ...
    'Preferred browser flow: use `browser_use` with `action="navigate"` for page loads, `action="snapshot"` for ARIA-tree reading, and `action="vision"` only when the visual layout matters.',
    'If snapshot content is incomplete, run `browser_use` with `action="scroll"` and then `action="snapshot"` again (repeat a few times for long/lazy-loaded pages).',
    'Do not use `browser_use` with `action="pdf"` as a text-reading step; PDF export is for artifacts, not extraction.',
    '',
