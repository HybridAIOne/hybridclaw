...
function normalizeCvEntry(value: unknown): AgentCvEntry | null {
  ...
  const outcome =
    ...
    value.outcome === 'failure'
      ? value.outcome
      : 'success';
  if (!runId || !sessionId || !skillId || !date || !createdAt || !title) {
    return null;
  }
  return {
    run_id: runId,
...
async function narrateSkillRuns(input: {
  ...
  const model = findCheapestModelMeetingCapabilities({});
...
function shouldRenderCv(input: {
  ...
  return (
    input.now.getTime() - new Date(input.state.last_rendered_at).getTime() >=
    input.throttleMs
  );
}
...
function addNarrationCost(input: {
  ...
  if (input.costUsd <= 0) return { ...input.costs };
  return {
    ...input.costs,
