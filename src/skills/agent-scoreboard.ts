...
const AGENT_CV_REFRESH_DEBOUNCE_MS = 30_000;
...
function queueAgentCvRefresh(agentId: string): void {
  queuedAgentCvRefreshWork = queuedAgentCvRefreshWork.then(() => {
    try {
      refreshAgentCv(agentId);
    } catch (error) {
      logger.warn(
        {
          agentId,
...
export function scheduleAgentCvRefresh(
  ...
  const delayMs = Math.max(
    0,
    Math.trunc(options?.delayMs ?? AGENT_CV_REFRESH_DEBOUNCE_MS),
  );
  const timer = setTimeout(() => {
    pendingAgentCvRefreshes.delete(normalizedAgentId);
    queueAgentCvRefresh(normalizedAgentId);
  }, delayMs);
  timer.unref?.();
...
export function refreshAgentCvForSkillRun(event: SkillRunEvent): void {
  ...
  scheduleAgentCvRefresh(event.agent_id);
}
