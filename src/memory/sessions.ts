...
export function forkSessionBranch(
  ...
  const fork = getSessionDatabase().transaction(() => {
    ...
    getSessionDatabase()
      .prepare(
        `INSERT INTO messages (session_id, user_id, username, role, agent_id, content, artifacts_json, activity_trace_json, routing_trace_json, tool_history_json, media_json, created_at)
       SELECT ?, user_id, username, role, agent_id, content, artifacts_json, activity_trace_json, routing_trace_json, tool_history_json, media_json, created_at
       FROM messages
       WHERE session_id = ?
         AND id < ?
       ORDER BY id ASC`,
