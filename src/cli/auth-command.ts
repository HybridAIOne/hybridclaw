...
async function resolveGenericProviderApiKey(
  ...
  const promptedApiKey = await promptForSecretInput({
    ...
    missingMessage: `Missing ${providerLabel} API key. Pass \`--api-key <key>\`, store it with \`/secret set <name> <value>\`, or run this command in an interactive terminal to paste it.`,
  });
  if (promptedApiKey) return promptedApiKey;
