...
async function resolveInteractiveTelegramSetup(params: {
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    throw new Error(
      'Missing Telegram bot token. Pass `--token <token>`, store TELEGRAM_BOT_TOKEN with `hybridclaw secret set TELEGRAM_BOT_TOKEN <token>` or in TUI with `/secret set TELEGRAM_BOT_TOKEN <token>`, or run this command in an interactive terminal to be prompted.',
    );
  }
