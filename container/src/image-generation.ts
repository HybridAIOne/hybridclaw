  ...
  const providers = [
    ...
    },
    {
      ...
      missing: ready.has('xai')
        ? null
        : 'Store XAI_API_KEY with `hybridclaw secret set XAI_API_KEY <key>` or in TUI with `/secret set XAI_API_KEY <key>`, or use a configured xai model.',
    },
    {
      id: 'bfl',
      label: 'Black Forest Labs',
      ...
      missing: ready.has('bfl')
        ? null
        : 'Store BFL_API_KEY or BLACK_FOREST_LABS_API_KEY with `hybridclaw secret set <name> <key>` or in TUI with `/secret set <name> <key>`.',
