...
test('unsubscribing stops queued judge work and suppresses sink delivery', async () => {
  const { initDatabase } = await import('../src/memory/db.js');
  initDatabase({ quiet: true, dbPath: createTempDbPath() });
  const { emitSkillRunEvent } = await import(
    '../src/skills/skill-run-events.js'
  ...
  const firstJudgeStarted = new Promise<void>((resolveStarted) => {
    releaseFirstJudge = resolveStarted;
  });
  const modelCaller = vi.fn(async () => {
    if (modelCaller.mock.calls.length === 1) {
  ...
  const sink = vi.fn();
  const unsubscribe = registerJudgeSubscriber({
    id: 'unsubscribed-feature',
    filter: () => true,
    criteria: 'Judge before unsubscribe.',
    sink,
    debounceMs: 0,
    judgeOptions: {
      model: 'judge-json-model',
  ...
  emitSkillRunEvent(buildSkillRunEvent({ run_id: 'run-1' }));
  emitSkillRunEvent(buildSkillRunEvent({ run_id: 'run-2' }));
  await vi.waitFor(() => {
    expect(modelCaller).toHaveBeenCalledTimes(1);
  });
