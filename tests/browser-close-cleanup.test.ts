...
function createCloseSuccessStub(root: string, logPath: string): string {
  const scriptPath = path.join(root, 'agent-browser-close-success-stub.mjs');
  fs.writeFileSync(
    scriptPath,
    `#!/usr/bin/env node
import fs from 'node:fs';
...
test('browser_close terminates the daemon when close reports success but the process remains alive', async () => {
  tempRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), 'hybridclaw-browser-close-success-fallback-'),
  );
  const logPath = path.join(tempRoot, 'close-success-log.jsonl');
  vi.stubEnv('HYBRIDCLAW_AGENT_WORKSPACE_ROOT', tempRoot);
  ...
  const snapshot = JSON.parse(
    await executeBrowserTool('browser_snapshot', {}, 'session-close-success'),
  ) as { success: boolean };
  expect(snapshot.success).toBe(true);
  ...
  const closed = JSON.parse(
    await executeBrowserTool('browser_close', {}, 'session-close-success'),
  ) as { success: boolean; closed: boolean; warning?: string };
