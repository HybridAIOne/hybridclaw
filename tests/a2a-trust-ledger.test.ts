...
describe('F7 trusted peer ledger', () => {
  test('persists webhook peers on the shared F7 substrate', async () => {
    const trust = await import('../src/a2a/trust-ledger.ts');
    ...
    const f7Peer = trust.getF7TrustedPeer('webhook', 'zapier-prod');
    expect(f7Peer).toMatchObject({
      schemaVersion: 1,
      transport: 'webhook',
      peerId: 'zapier-prod',
      senderAgentId: 'remote@team@peer-instance',
    ...
    expect(trust.listF7TrustedPeers('webhook')).toHaveLength(1);
  ...
  test('migrates legacy webhook peers without breaking reads', async () => {
    ...
    const f7Peer = trust.getF7TrustedPeer('webhook', 'legacy-zapier');
    expect(f7Peer).toMatchObject({
      transport: 'webhook',
      peerId: 'legacy-zapier',
      trust: {
    ...
    expect(trust.listF7TrustedPeers('webhook')).toEqual([f7Peer]);
  });

  test('resolves A2A JSON-RPC peers from shared F7 records', async () => {
    const trust = await import('../src/a2a/trust-ledger.ts');
    ...
    const f7Peer = trust.getF7TrustedPeer('a2a', 'instance-b');
    expect(f7Peer).toMatchObject({
      transport: 'a2a',
      peerId: 'instance-b',
      senderAgentId: 'remote@team@inst-b',
  ...
  test('mirrors public-key TOFU trust into F7 A2A peer records', async () => {
    const { initDatabase } = await import('../src/memory/db.ts');
    const trust = await import('../src/a2a/trust-ledger.ts');
    const publicKeyJwk = peerPublicKeyJwk();
    const key = {
    ...
    const f7Peer = trust.getF7TrustedPeer('a2a', 'tofu-peer');
    expect(f7Peer).toMatchObject({
      transport: 'a2a',
      peerId: 'tofu-peer',
      trust: {
  ...
  test('migrates legacy public-key trust records into F7', async () => {
    const revisions = await import('../src/config/runtime-config-revisions.ts');
    const runtimePaths = await import('../src/config/runtime-paths.ts');
    const trust = await import('../src/a2a/trust-ledger.ts');
    const publicKeyJwk = peerPublicKeyJwk();
    ...
    expect(trust.getF7TrustedPeer('a2a', 'legacy-public-key')).toMatchObject({
      transport: 'a2a',
      peerId: 'legacy-public-key',
      auditLineage: {
        origin: 'tofu',
  ...
  test('merges legacy A2A JSON-RPC records into existing F7 A2A peers', async () => {
    const { initDatabase } = await import('../src/memory/db.ts');
    const revisions = await import('../src/config/runtime-config-revisions.ts');
    const runtimePaths = await import('../src/config/runtime-paths.ts');
    const trust = await import('../src/a2a/trust-ledger.ts');
    ...
    expect(
      trust.getF7TrustedA2AJsonRpcPeerBySender(
    ...
    expect(trust.getF7TrustedPeer('a2a', 'merged-peer')).toMatchObject({
      transport: 'a2a',
      peerId: 'merged-peer',
      a2a: {
        publicKeyPem,
