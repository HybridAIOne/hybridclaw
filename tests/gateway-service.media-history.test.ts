...
test('getGatewayHistory falls back to legacy web response ratings', async () => {
  setupHome();
  ...
  const sessionId = 'web:legacy-response-rating-history';
  getOrCreateSession(sessionId, null, 'web');
  storeMessage(sessionId, 'user-1', 'web', 'user', 'Rate this');
  ...
  const history = getGatewayHistory(sessionId, 10, {
...
test('getGatewayHistory prefers operator-specific ratings over legacy web ratings', async () => {
  setupHome();
  ...
  const sessionId = 'web:specific-response-rating-history';
  getOrCreateSession(sessionId, null, 'web');
  storeMessage(sessionId, 'user-1', 'web', 'user', 'Rate this');
  const assistantMessageId = storeMessage(
    sessionId,
