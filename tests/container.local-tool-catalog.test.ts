...
describe('local tool catalog boundary', () => {
  ...
  test('uses the configured selection, including catalog-only mode', () => {
    expect(new ToolCatalog(available, ['memory']).tools.map((entry) => entry.function.name)).toEqual(['memory', 'tool_catalog']);
    expect(new ToolCatalog(available, []).tools.map((entry) => entry.function.name)).toEqual(['tool_catalog']);
  });
  test('never restores filtered tools, including direct and replayed calls', () => {
    const catalog = new ToolCatalog([tool('read'), tool('memory')], ['bash']);
    ...
    expect(new ToolCatalog([]).tools).toEqual([]);
    expect(() => new ToolCatalog([]).resolveCall(catalogCall({ action: 'list' }))).toThrow('not available');
  });
  ...
  test.each([
    ...
    expect(() => new ToolCatalog(available).resolveCall(catalogCall(args))).toThrow();
  });
  test('bounds pages, descriptions and individual schema output', () => {
    ...
    const huge = new ToolCatalog([tool('huge', 'x'.repeat(25_000))]);
  ...
  test('respects disabling discovery and rejects a reserved name collision', () => {
    expect(new ToolCatalog(available, ['read'], true).tools.map((entry) => entry.function.name)).toEqual(['read']);
    expect(() => new ToolCatalog([tool('tool_catalog')])).toThrow('reserved');
  });
...
test('prompt guidance reflects actual exposed schemas without granting hidden tools', () => {
  ...
  const compact = new ToolCatalog(available, ['skills_list']);
  ...
  const direct = new ToolCatalog(available, ['read']);
  expect(direct.promptGuidance()).not.toContain('Never emit a direct read call');
  const noDiscovery = new ToolCatalog(available, ['skills_list'], true);
  ...
  const restricted = new ToolCatalog([tool('skills_list')], []).promptGuidance();
  expect(restricted).not.toContain('name":"bash');
  expect(restricted).not.toContain('name":"read');
  expect(new ToolCatalog([]).promptGuidance()).toContain('No functions are exposed');
});


test('returns corrective lookup errors without echoing unknown names or restoring tools', () => {
  const catalog = new ToolCatalog([tool('read'), tool('skills_list')], ['skills_list']);
  ...
  const noRead = new ToolCatalog([tool('memory')], []);
...
test('describes the exact catalog invocation without exposing another function', () => {
  ...
  const catalog = new ToolCatalog([read], []);
  const before = JSON.stringify(catalog.tools);
...
test('validates nested arguments before resolving an action without coercion or payload echoes', () => {
  ...
  for (const args of [{ path: 'private-placeholder' }, { command: 123, options: { mode: 'safe' } }, { command: 'pwd', options: { mode: 'wrong' } }]) {
    const catalog = new ToolCatalog([target], []);
  ...
  expect(new ToolCatalog([target], []).resolveCall(original).function.arguments).toBe(JSON.stringify(args));
  expect(original.function.name).toBe('tool_catalog');
});
...
test('isolates schema ids and refuses schemas requiring external resolution', () => {
  ...
  const catalog = new ToolCatalog([a, b], []);
  ...
  const blocked = new ToolCatalog([external], []);
...
describe('deferred MCP tools behind the catalog', () => {
  const builtins = [tool('read'), tool('bash'), tool('skills_list')];
  const mcp = [tool('crm__search', 'Search CRM contacts'), tool('crm__create_note')];
  ...
  test('keeps every non-MCP tool exposed and routes MCP tools through tool_catalog', () => {
    ...
    expect(listed.availableCount).toBe(2);
    expect(catalog.discoveryResult(catalogCall({ action: 'describe', name: 'crm__search' }))?.output).toContain('Search CRM contacts');
    const resolved = catalog.resolveCall(catalogCall({ action: 'call', name: 'crm__search', arguments: {} }));
    expect(resolved.function.name).toBe('crm__search');
    expect(catalog.resolveCall(call('bash', { command: 'pwd' })).function.name).toBe('bash');
    // Hiding a schema never revokes permission: a direct call still resolves, as in local mode.
