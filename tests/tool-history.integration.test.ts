...
test('upgrades schema 58 without losing chat or scheduler failure data', async () => {
  const { runMigrations, DATABASE_SCHEMA_VERSION } = await import('../src/memory/schema/migrations.js');
  ...
  try {
    ...
    expect(database.pragma('user_version', { simple: true })).toBe(DATABASE_SCHEMA_VERSION);
    expect(
