...
export class HonchoRuntime {
  ...
  async onSessionEnd(context) {
    await this.flushBufferedMessages(context.sessionId);
  }
  ...
  resolveSessionContext(params) {
    const sessionInfo = this.api.getSessionInfo(params.sessionId || '');
    const userId =
    ...
    const agentId =
      ...
      'main';
    const sessionRoot =
      normalizeString(sessionInfo.workspacePath) || this.api.runtime.cwd;
    const honchoSessionId = sanitizeHonchoSessionId(
      resolveHonchoSessionKey({
        config: this.config,
        cwd: sessionRoot,
        platformSessionId: params.sessionId,
      }),
    );
    return {
      platformSessionId: normalizeString(params.sessionId),
      honchoSessionId,
      userId,
      agentId,
