...
| 1 | **Agent-to-agent messaging** | First-class primitive for agents to message, hand off, escalate, federate across instances, and speak multiple transport formats. See **R1 Messaging Work** below for sub-issue rows. *Principle VI.* | P0 | 🟡 2/15 |
...
| R1.10 | Transport | Transport registry and peer descriptor schema | ⬜ #765 |
| R1.11 | Transport | A2A outbound adapter | ⬜ #766 |
| R1.12 | Transport | A2A inbound adapter and Agent Card endpoint | ⬜ #767 |
| R1.13 | Transport | Signed-HMAC webhook outbound adapter with retry and jitter | ⬜ #768 |
...
| R30.2 | Local provider | Local Playwright provider using persistent browser context | ⬜ To be filed |
| R30.3 | Managed provider | Managed-cloud browser provider with shared navigation guard | ⬜ To be filed |
| R30.4 | Passthrough provider | Browser-use cloud passthrough with metering and audit URLs | ⬜ To be filed |
...
- 🟡 **F7** — Global identity primitives. Agent-ID format is in production in [`src/a2a/identity.ts`](../../../src/a2a/identity.ts); canonical user IDs, trust, and cross-instance auth remain. See **Foundation Sub-Issues** below for F7 rows.
...
| F6.5 | Public URL | Deployment-mode and public-URL docs | ⬜ To be filed |
...
| F7.1 | Identity | Canonical user IDs | ⬜ #571 |
