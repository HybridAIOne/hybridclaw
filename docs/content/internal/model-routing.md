...
By default, no remote LLM call runs on the routing hot path. Phase 2 permits an opt-in, bounded JEV evaluation for exact admin-approved public prompts after local disclosure checks; shadow mode never changes execution. Never break a pin, a privacy limit, or a stakes/approval policy to save money. No quality-signal gating before telemetry is calibrated. No hardcoded tier names, tier count, or hardware anywhere in code. The OSS router is fully functional standalone (any two self-hosted endpoints form a ladder); the enterprise layer sells fleet operations, policy, dashboards, and guarantees — not the mechanism.
