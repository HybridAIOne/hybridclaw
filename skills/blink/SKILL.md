...
- Stop after a 426 `app update is required` response. Do not probe alternate Blink endpoints; update the helper default or set `BLINK_USER_AGENT` / `--user-agent` to a current Blink app identity.
...
If Blink rejects login with `{"message":"An app update is required"}`, the
mobile-app identity is stale. Use a current Blink Home Monitor version and run:

```bash
BLINK_USER_AGENT="BlinkHomeSecurity/<version> (iPhone; iOS 17.6; Scale/3.00)" \
  node skills/blink/blink.cjs --format json http-request account-login
```
...
1. Run `http-request account-login` if `BLINK_AUTH_TOKEN`, `BLINK_TIER`, `BLINK_ACCOUNT_ID`, or `BLINK_CLIENT_ID` is missing or stale.
2. If the login response or gateway result reports status 401/412, `verification_required`, `client_verification_required`, or similar invalid-credential text, stop immediately; for verification challenges, ask for F14 PIN handover and run `http-request pin-verify --pin <code>`.
3. If the login response reports status 426 or `app update is required`, stop immediately and update the helper User-Agent. Do not try guessed `/api/v3`, `/api/v4`, `/api/v6`, or OAuth paths.
4. Use `devices-list` first for a compact account overview; it includes networks, sync modules, cameras, and doorbell-like devices on current Blink accounts.
